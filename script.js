...
function renderTasks() {
  ...
  const buildItem = (t) => `
    <div class="task-item" style="opacity: ${t.done ? 0.5 : 1}">
      <div class="task-icon" style="background: ${t.prio === 'Overdue' ? 'rgba(239, 68, 68, 0.1)' : t.prio === 'High' ? 'rgba(245, 158, 11, 0.1)' : t.prio === 'Medium' ? 'rgba(59, 130, 246, 0.1)' : 'rgba(16, 185, 129, 0.1)'}">
        <i data-lucide="${t.done ? 'check-circle-2' : (t.prio === 'Overdue' ? 'alert-triangle' : 'circle')}" color="${t.prio === 'Overdue' ? '#ef4444' : t.prio === 'High' ? '#f59e0b' : t.prio === 'Medium' ? '#3b82f6' : '#10b981'}"></i>
      </div>
      <div class="task-info">
        <div style="font-weight: 700; font-size: 1.05rem; text-decoration: ${t.done ? 'line-through' : 'none'}">${t.name}</div>
        <span style="font-size: 0.8rem; color: var(--text-secondary); font-weight: 500;">Due: ${new Date(t.date).toLocaleDateString()}</span>
      </div>
